...
# Buffer the points
df_buf <- st_buffer(df_crs, dist = 1000) # the unit is by default in meters
...
beech_box <- st_make_grid(beech, n = 1) # create the smallest bow that can fit all the beech trees in NY

# Plot the neighborhoods, add the beech trees and add the new box
plot(st_geometry(neighborhoods), col = "grey", border = "white")
plot(beech, add = TRUE, pch = 16, col = "forestgreen")
plot(st_geometry(beech_box), add = TRUE)
...
summary(beech_buf_union)

# Plot the dissolved buffers
plot(beech_buf_union)

# st_union dissolves all the polygons into one - so the circles become a 'cloud'
# less data = easier to run
...
beech1 <- st_union(beech) # the 'geometry type' is point

# Look at the data frame to see the type of geometry
head(beech1) # the 'geometry type' is multipoint
