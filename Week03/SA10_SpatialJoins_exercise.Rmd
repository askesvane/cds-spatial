...
plot(empire_state, add = T, col = "blue")
...
```{r}
class(neighborhoods_int)
# the class of the object is a list
```

2. Why do we need to use `neighborhoods[cont,]` to view the fully contained neighborhood in the buffer plot?
It extracts the single row 'cont' which is 79, and takes all columns. It adds a layer colored yellow so we can easily see the neighborhood.
