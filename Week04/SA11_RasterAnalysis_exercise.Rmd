...
# Read in the manhattan Landsat image multi-band raster (so we use brick() to import)
...
# Checking whether I have done it right 
st_crs(parks_cp) == st_crs(canopy)
...
# Convert parks from vector object (multipolygon) to a Spatial object
# currently the `raster` package does not support `sf` objects so they will need to be converted to `Spatial` objects with, for example, `as(input, "Spatial")`
parks_sp <- as(parks_big, "Spatial")


# Mask the canopy layer with parks_sp and save as canopy_mask
# we use the spatial object of parks to mask the canopy object. Meaning that we will only have the canopies wihtin the park areas
...
# Plot the geometry of parks_big
# we need to use st_geometry because parks_big is an sf object. parks_sp is a spatial object (see last chunk)
plot(st_geometry(parks_big))
...
# Empire state building = 0.00
# Mesuem of natural history = 18.22
# Central park = 63.93
...
# there is a big white spot at Central Park indicating that I have done it right :)
```
