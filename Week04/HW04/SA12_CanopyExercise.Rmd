...
# Now, we have an extra column in the neighborhoods_area df
...
neighborhoods_counts <- mutate(neighborhoods_counts, 
                            tree_cnt = ifelse(is.na(tree_cnt), 
                                              0, tree_cnt))
# This replaced 7 NA's with 0's.

# Compute the density
neighborhoods_counts <- mutate(neighborhoods_counts, 
                               tree_density = tree_cnt/area)
# This will compute tree per m^2 which will obviously be very small numbers.
```
...
# TRUE means that they now have the same crs.
...
# a negative correlation of -0.3414462
```
...
# I have used tm_rgb() instead of tm_raster() as this function will stack the three layers RGB on top of each other
...
# the tree density map
manhattan_box <- st_bbox(manhattan) # Create a boundign box based on the manhattan raster
...
# Plot the three maps together to enhance comparability
tmap_arrange(map1, map2, map3, asp = NA)

# Obviously, by looking at central park canopy appears to be a better measure of to what extent an area is green.
